...
class DocsSidebar extends StatelessComponent {
  ...
  Component _buildItem(
    ...
    if (!hasChildren) {
      return li(classes: 'sidebar-leaf sidebar-depth-$depth', [
        DocsNavLink(
          to: item.href ?? '#',
    ...
    return li(classes: 'sidebar-section sidebar-depth-$depth', [
      details(
        classes: isOpen ? 'sidebar-details open' : 'sidebar-details',
        attributes: {if (isOpen) 'open': 'open'},
        [
          summary(classes: triggerClasses, [
            span(classes: 'sidebar-trigger-label', [Component.text(item.text)]),
            span(
  ...
  bool _matchesRoute(String? href, String activeRoute) {
    ...
    if (normalizedHref == '/api' && activeRoute.startsWith('/api/'))
      return true;
    ...
    css('.sidebar', [
      ...
      downContent([
        css('&').styles(
          ...
          raw: {'width': 'var(--docs-shell-drawer-width)'},
        ),
      ]),
      css('.sidebar-close', [
        css('&').styles(
        ...
        downContent([css('&').styles(display: Display.block)]),
      ]),
      css('.sidebar-group', [
        css('&').styles(
        ...
        css('.sidebar-details').styles(raw: {'display': 'block'}),
        css(
          '.sidebar-details > summary',
        ).styles(listStyle: ListStyle.none, cursor: Cursor.pointer),
        css(
        ...
        css('.sidebar-trigger').styles(
          ...
          padding: Padding.symmetric(vertical: 0.48.rem, horizontal: Unit.zero),
          color: ContentColors.text,
        ...
        css('.sidebar-trigger:hover').styles(color: ContentColors.primary),
        css('.sidebar-trigger-label').styles(raw: {'min-width': '0'}),
        css('.sidebar-trigger-chevron').styles(
          ...
          raw: {'line-height': '1', 'transform': 'rotate(90deg)'},
        ),
        css(
          '.sidebar-details[open] > .sidebar-trigger .sidebar-trigger-chevron',
        ).styles(opacity: 1, raw: {'transform': 'rotate(270deg)'}),
        css(
          '.sidebar-trigger.active-branch',
        ).styles(color: ContentColors.primary),
        ...
        css(
          '.sidebar-sublist.root-sublist',
        ).styles(margin: Margin.only(left: 0.1.rem)),
        css('.sidebar-link').styles(
          opacity: 0.92,
          display: Display.block,
          margin: Margin.zero,
          padding: Padding.symmetric(vertical: 0.45.rem, horizontal: Unit.zero),
          ...
          raw: {'line-height': '1.35', 'white-space': 'normal'},
        ),
        css(
          '.sidebar-link:hover',
        ).styles(opacity: 1, color: ContentColors.primary),
        ...
        css('.sidebar-depth-1 > .sidebar-link').styles(fontSize: 0.95.rem),
        css(
          '.sidebar-depth-2 > .sidebar-link',
        ).styles(fontSize: 0.92.rem, opacity: 0.9),
        css(
          '.sidebar-group-libraries',
        ).styles(margin: Margin.only(top: 0.2.rem)),
        css('.sidebar-group-libraries .sidebar-link').styles(
          fontSize: 0.94.rem,
          opacity: 0.86,
          padding: Padding.symmetric(vertical: 0.46.rem, horizontal: Unit.zero),
        ),
        css('.sidebar-group-libraries .sidebar-link.active').styles(opacity: 1),
      ]),
    ]),
  ];
}
