...
List<DartdocOption<Object?>> createSourceLinkerOptions(
  ...
  return [
    DartdocOptionSet('linkToSource', resourceProvider)..addAll([
      DartdocOptionArgFile<List<String>>(
        ...
        help:
            'A list of directories to exclude from linking to a source '
            'code repository.',
      ),
...
class SourceLinker {
  ...
  String href() {
    ...
    if (!path.isWithin(root, sourceFileName) ||
        excludes.any(
          (String exclude) => path.isWithin(exclude, sourceFileName),
        )) {
      return '';
    }
    return uriTemplate.replaceAllMapped(_uriTemplateRegExp, (match) {
      return switch (match[1]) {
        '%f%' => path.url.joinAll(
          path.split(path.relative(sourceFileName, from: root)),
        ),
        '%r%' => revision!,
