...
/// Computes Jaspr-compatible file paths and URLs for documentation elements.
///
...
class JasprPathResolver {
  ...
  String? filePathFor(Documentable element) {
    if (element is Package) {
      return 'content/api/index.md';
    }

    if (element is Category) {
      return 'content/topics/${sanitizeFileName(element.name)}.md';
    }

    if (element is Library) {
      if (path_utils.isInternalSdkLibrary(element)) return null;
      return 'content/api/${_resolvedDirName(element)}/index.md';
    }
    ...
    if (element is ModelElement) {
      ...
      return 'content/api/$dirName/$fileName.md';
    }
  ...
  /// relative paths for HTML type-links (Jaspr does not rewrite `<a href>`
  ...
  String? anchorFor(ModelElement element) {
    ...
    // collision with Jaspr auto-generated ID for the class H1 heading.
  ...
  String? _libraryDirName(Documentable element) {
    ...
    if (element is ModelElement) {
      ...
      if (path_utils.isInternalSdkLibrary(lib)) return null;
...
// isDuplicateSdkLibrary and isInternalSdkLibrary are now in
// core/path_utils.dart and re-exported from this file.
