...
class VitePressSidebarGenerator {
  ...
  String generate(PackageGraph packageGraph) {
    ...
    buf.writeln('// AUTO-GENERATED by dartdoc_modern -- do not edit manually.');
  ...
  void _generateFlat(
    ...
    buf.writeln('export const apiSidebar: DefaultTheme.Sidebar = {');
    buf.writeln("  '/api/': [");
    ...
    for (final package in packages) {
      ...
      if (isMultiPackage) {
        if (libraries.length == 1 && libraries.first.name == package.name) {
          ...
          for (final library in otherLibs) {
            final dirName = paths.dirNameFor(library);
            final escapedName = _escapeTs(
              displayNames[library] ?? library.name,
            );
        ...
        for (final library in libraries) {
          final dirName = paths.dirNameFor(library);
          final escapedName = _escapeTs(displayNames[library] ?? library.name);
          final text = library.isDeprecated
              ? '<s>$escapedName</s>'
              : escapedName;
          buf.writeln('    {');
  ...
  void _writeCategoryGroups(
    ...
    for (final category in categories) {
      ...
      final libs =
          category.publicLibrariesSorted
