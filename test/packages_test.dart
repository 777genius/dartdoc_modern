...
void main() {
  ...
  setUp(() async {
    ...
    optionSet = DartdocOptionRoot.fromOptionGenerators('dartdoc', [
...
''');
        ...
        test('has a homepage', () async {
          ...
          expect(
            packageGraph.defaultPackage.homepage,
...
''',
          ...
          expect(
            packageGraph.libraries.where((lib) => lib.name == 'b'),
            hasLength(1),
          );
        });
        ...
        test('has documentation via Markdown README', () async {
          ...
          expect(
            packageGraph.defaultPackage.packageMeta.getReadmeContents(),
            isNotNull,
          );
        ...
        test('has documentation content rendered as HTML', () async {
          ...
          expect(
            packageGraph.defaultPackage.documentationAsHtml,
...
''',
        ...
        expect(
          documentedLibraries.map((e) => e.name),
          containsAll(['foo', 'bar']),
        );
...
''');
        ...
        test(
          'includes remote elements when linkTo -> url is specified',
          () async {
            packageOneRoot
...
''');
            ...
            var packageOne = packageGraph.packages.singleWhere(
              (p) => p.name == 'one',
            );
            // TODO(srawlins): Why is there more than one?
            var libraryOne = packageOne.allLibraries.lastWhere(
              (l) => l.name == 'one',
            );
...
''',
        ...
        test(
          'package with no homepage in the pubspec has no homepage',
          () async {
            projectRoot = utils.writePackage(
              packageName,
              resourceProvider,
...
''',
      ...
      group('ordering', () {
        ...
        Folder bootBasicPackages(List<String> names) {
          var dependencies = {for (var name in names) name: '../../$name'};
...
''');
        ...
        test(
          'default package precedes SDK which precedes dependencies',
          () async {
  ...
  void test_exportedElements() async {
    var graph = await bootPackageFromFiles([
      d.dir('lib', [
        ...
        d.dir('src', [d.file('impl.dart', 'class C {}')]),
      ]),
    ]);
