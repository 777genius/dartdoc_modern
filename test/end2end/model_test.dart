...
import 'package:dartdoc_modern/src/render/parameter_renderer.dart';
...
void main() async {
  ...
  group('triple-shift', () {
    ...
    setUpAll(() async {
      ...
      classShift = C.instanceOperators.firstWhere(
        (o) => o.name.contains('>>>'),
      );
      extensionShift = ShiftIt.instanceOperators.firstWhere(
        (o) => o.name.contains('>>>'),
      );
    ...
    test(
      ...
      () {
        var tripleShiftE = E.instanceOperators.firstWhere(
          (o) => o.name.contains('>>>'),
        );
        var tripleShiftF = F.instanceOperators.firstWhere(
          (o) => o.name.contains('>>>'),
        );
  ...
  group('generic metadata', () {
    ...
    setUpAll(() async {
      genericMetadata = (await testPackageGraph).libraries.named(
        'generic_metadata',
      );
    ...
    test(
      ...
      () {
        expect(
          (F.aliasedType as FunctionType).typeParameters.first.metadata,
          isNotEmpty,
        );
        expect(
          (F.aliasedType as FunctionType).typeParameters.first.metadata,
    ...
    test('Verify type arguments on annotations renders, including parameters', () {
      ...
      expect(
        genericMetadata.annotations.first.linkedNameWithParameters,
        equals(ab0),
      );
      ...
      expect(
        mp.parameters.map((p) => p.annotations.first.linkedNameWithParameters),
        everyElement(equals(ab0)),
      );
      expect(
        mn.parameters.map((p) => p.annotations.first.linkedNameWithParameters),
        everyElement(equals(ab0)),
      );
    });
  });
  ...
  group('generalized typedefs', () {
    ...
    setUpAll(() {
      generalizedTypedefs = packageGraph.libraries.named(
        'generalized_typedefs',
      );
    ...
    void expectTypedefs(
      ...
      expect(
        t.element.typeParameters.map((p) => p.toString()),
    ...
    test('typedef references display aliases', () {
      ...
      expectAliasedTypeName(
        d.parameters.first.modelType as Aliased,
        equals('T3'),
      );
      expectAliasedTypeName(
        d.parameters.last.modelType as Aliased,
        equals('T4'),
      );

      expectAliasedTypeName(g.modelType.returnType as Aliased, equals('T1'));
      expectAliasedTypeName(
        g.modelType.parameters.first.modelType as Aliased,
        equals('T2'),
      );
      expectAliasedTypeName(
        g.modelType.parameters.last.modelType as Aliased,
        equals('T3'),
      );
    });

    test(
      'typedef references to special types work',
  ...
  group('NNBD cases', () {
    ...
    setUpAll(() async {
      lateFinalWithoutInitializer = packageGraph.libraries.named(
        'late_final_without_initializer',
      );
      nullSafetyClassMemberDeclarations = packageGraph.libraries.named(
        'nnbd_class_member_declarations',
      );
      ...
      oddAsyncFunction = nullableElements.functions.wherePublic.named(
        'oddAsyncFunction',
      );
      anotherOddFunction = nullableElements.functions.wherePublic.named(
        'oddAsyncFunction',
      );
      neverReturns = nullableElements.functions.wherePublic.named(
        'neverReturns',
      );
      almostNeverReturns = nullableElements.functions.wherePublic.named(
        'almostNeverReturns',
      );
    });
    ...
    test('Never types are allowed to have nullability markers', () {
      ...
      expect(
        almostNeverReturns.modelType.returnType.nullabilitySuffix,
        equals('?'),
      );
    });
    ...
    test('old implied Future types have correct nullability', () {
      ...
      expect(
        oddAsyncFunction.modelType.returnType.nullabilitySuffix,
        equals(''),
      );
      expect(anotherOddFunction.modelType.returnType.name, equals('dynamic'));
      expect(
        anotherOddFunction.modelType.returnType.nullabilitySuffix,
        equals(''),
      );
    });
    ...
    test('Late final top level variables', () {
      ...
      expect(
        initializeMe.attributes,
        isNot(contains(Attribute.getterSetterPair)),
      );
    });

    test('complex nullable elements are detected and rendered correctly', () {
      var complexNullableMembers = nullableElements.classes.named(
        'ComplexNullableMembers',
      );
      expect(
    ...
    test('simple nullable elements are detected and rendered correctly', () {
      ...
      var operatorStar = nullableMembers.instanceOperators.wherePublic.named(
        'operator *',
      );
      expect(
  ...
  group('Set Literals', () {
    ...
    setUpAll(() async {
      ...
      untypedMapWithoutConst = set_literals.constants.named(
        'untypedMapWithoutConst',
      );
    ...
    test('Set literals test', () {
      ...
      expect(
        (aComplexSet.modelType as ParameterizedElementType).typeArguments
            .map((a) => a.name)
            .toList(),
        equals(['AClassContainingLiterals']),
      );
      expect(
        aComplexSet.constantValue,
        equals('const {const AClassContainingLiterals(3, 5)}'),
      );
      expect(inferredTypeSet.modelType.name, equals('Set'));
      expect(
        (inferredTypeSet.modelType as ParameterizedElementType).typeArguments
            .map((a) => a.name)
            .toList(),
        equals(['int']),
      );
      ...
      expect(
        (specifiedSet.modelType as ParameterizedElementType).typeArguments
            .map((a) => a.name)
            .toList(),
        equals(['int']),
      );
      ...
      // `const` here.
      expect(
        untypedMapWithoutConst.constantValue,
        matches(RegExp('(const )?{}')),
      );
      expect(untypedMap.modelType.name, equals('Map'));
      expect(
        (untypedMap.modelType as ParameterizedElementType).typeArguments
            .map((a) => a.name)
            .toList(),
      ...
      expect(
        (typedSet.modelType as ParameterizedElementType).typeArguments
            .map((a) => a.name)
            .toList(),
        equals(['String']),
      );
      expect(
        typedSet.constantValue,
  ...
  group('Tools', () {
    ...
    final RegExp packageInvocationIndexRegexp = RegExp(
      r'PACKAGE_INVOCATION_INDEX: (\d+)',
    );

    setUpAll(() {
      ImplementingClassForTool = fakeLibrary.classes.named(
        'ImplementingClassForTool',
      );
      CanonicalPrivateInheritedToolUser = fakeLibrary.classes.named(
        'CanonicalPrivateInheritedToolUser',
      );
      ...
      invokeToolMultipleSections = toolUser.instanceMethods.named(
    ...
    test(
      ...
      () {
        ...
        expect(
          invokeToolParentDoc.documentationFrom,
    ...
    test('can invoke a tool and pass args and environment', () {
      ...
      expect(
        invokeTool.documentation,
        contains('--package-name=test_package, '),
      );
      ...
      expect(
        invokeTool.documentation,
        contains('PACKAGE_NAME: test_package, '),
      );
    ...
    test('can invoke a tool multiple times in one comment block', () {
      ...
      expect(
        envLine.allMatches(invokeToolMultipleSections.documentation),
        hasLength(2),
      );
      var argLine = RegExp(r'^Args: \[', multiLine: true);
      expect(
        argLine.allMatches(invokeToolMultipleSections.documentation),
        hasLength(2),
      );
      expect(
        invokeToolMultipleSections.documentation,
  ...
  group('Missing and Remote', () {
    test(
      ...
      () {
        expect(
          dartAsync.package.documentedWhere,
          equals(DocumentLocation.missing),
        );
    ...
    test(
      'Verify that packageGraph has an SDK but will not document it locally',
      () {
        expect(
          packageGraph.packages.firstWhere((p) => p.isSdk).documentedWhere,
  ...
  group('Category', () {
    test('Verify categories for test_package', () {
      ...
      expect(
        packageGraph.localPackages.first.categoriesWithPublicLibraries,
        hasLength(3),
      );
      expect(
    ...
    test(
      'Verify libraries with multiple categories show up in multiple places',
      () {
        var packageCategories = packageGraph.publicPackages.first.categories;
        var realLibraries = packageCategories.firstWhere(
          (c) => c.name == 'Real Libraries',
        );
        var misc = packageCategories.firstWhere((c) => c.name == 'Misc');
        expect(
          realLibraries.libraries.map((l) => l.name),
          contains('two_exports'),
        );
  ...
  group('Library', () {
    ...
    test('can be reexported even if the file suffix is not .dart', () {
      expect(
        fakeLibrary.classes.map((c) => c.name),
    ...
    test('exported function', () {
      expect(
        exLibrary.functions.any((f) => f.name == 'helperFunction'),
        isFalse,
      );
    });
    ...
    test('can import other libraries with unusual URIs', () {
      final importLists = fakeLibrary.element.fragments.map(
        (fragment) => fragment.libraryImports,
      );
      final exportLists = fakeLibrary.element.fragments.map(
        (fragment) => fragment.libraryExports,
      );
      ...
      expect(
        fakeLibraryImportedExported.any((l) => l.name == 'import_unusual'),
        isTrue,
      );
    });
  ...
  group('Macros', () {
    ...
    setUpAll(() {
      ...
      macroReferencedHere =
          MacrosFromAccessors.publicEnumValues.named('macroReferencedHere')
              as EnumField;
    ...
    test('via reexport does not leave behind template crumbs', () {
      expect(
        ...
        equals('I had better not have a template directive in my one liner.'),
      );
    });

    test('renders a macro defined within a enum', () {
      expect(
        macroReferencedHere.documentationAsHtml,
    ...
    test("renders a macro within the same comment where it's defined", () {
      expect(
        withMacro.documentation,
        equals('Macro method\n\nFoo macro content\n\nMore docs'),
      );
    });

    test(
      "renders a macro in another method, not the same where it's defined",
      () {
  ...
  group('YouTube', () {
    ...
    setUpAll(() {
      ...
      withYouTubeInOneLineDoc = dog.instanceMethods.named(
  ...
  group('Animation', () {
    ...
    setUpAll(() {
      ...
      withAnimationInOneLineDoc = dog.instanceMethods.named(
  ...
  group('Docs as HTML', () {
    ...
    setUpAll(() {
      incorrectDocReferenceFromEx = exLibrary.constants.named(
      ...
      testingCodeSyntaxInOneLiners = fakeLibrary.constants.named(
      ...
      doAwesomeStuff = baseForDocComments.instanceMethods.named(
        'doAwesomeStuff',
      );
    ...
    group('doc references', () {
      ...
      test('Verify links to inherited members inside class', () {
        ...
        var doAwesomeStuffWarnings =
            packageGraph.packageWarningCounter.countedWarnings[doAwesomeStuff
                .element] ??
            {};
        expect(
          doAwesomeStuffWarnings,
          isNot(
            doAwesomeStuffWarnings.containsKey(
      ...
      test('can handle renamed imports', () {
        var aFunctionUsingRenamedLib = fakeLibrary.functions.named(
      ...
      test(
        'operator [] reference outside of a class works',
      ...
      test(
        'links to a top-level variable with a prefix from an imported lib',
        () {
          expect(
            docsAsHtml,
            contains(
    ...
    test('still has brackets inside code blocks', () {
      expect(
        topLevelFunction.documentationAsHtml,
        contains("['hello from dart']"),
      );
    });

    test(
      'oneLine doc references in inherited methods should not have brackets',
      () {
    ...
    test('incorrect doc references are still wrapped in code blocks', () {
      expect(
        incorrectDocReferenceFromEx.documentationAsHtml,
    ...
    test('reference to constructor named the same as a field', () {
      var FieldAndCtorWithSameName = exLibrary.classes.named(
        'FieldAndCtorWithSameName',
      );
  ...
  group('Class edge cases', () {
    ...
    test(
      ...
      () {
        var implementorsLibrary = packageGraph.publicLibraries.named(
          'implementors',
        );
        var ImplementerOfDeclaredPrivateClasses = implementorsLibrary.classes
            .named('ImplementerOfDeclaredPrivateClasses');
        var ImplementerOfThings = implementorsLibrary.classes.named(
          'ImplementerOfThings',
        );
    ...
    test(
      'Overrides from intermediate abstract classes are picked up correctly',
      () {
        var IntermediateAbstractSubclass = fakeLibrary.classes.named(
          'IntermediateAbstractSubclass',
        );
        var operatorEquals = IntermediateAbstractSubclass.inheritedOperators
            .named('operator ==');
        expect(
          operatorEquals.definingEnclosingContainer.name,
    ...
    test(
      ...
      () {
        ...
        var operatorEqualsInt = intClass.inheritedOperators.named(
          'operator ==',
        );
        expect(
          operatorEqualsInt.definingEnclosingContainer.name,
          equals('num'),
        );
      },
    );
    ...
    test(
      'Inherit from private class across private library to public library',
      () {
  ...
  group('Mixin', () {
    ...
    test('computes interfaces and implementors correctly', () {
      var ThingToImplementInMixin = fakeLibrary.classes.wherePublic.named(
        'ThingToImplementInMixin',
      );
      var MixedInImplementation = fakeLibrary.classes.wherePublic.named(
        'MixedInImplementation',
      );
    ...
    test('Verify inheritance/mixin structure and type inference', () {
      expect(
        TypeInferenceMixedIn.mixedInTypes.map<String>(
          (element) => element.name,
        ),
        orderedEquals(['GenericMixin']),
      );
      expect(
        TypeInferenceMixedIn.mixedInTypes.first.typeArguments.map<String>(
          (ElementType t) => t.name,
        ),
        orderedEquals(['int']),
      );
      ...
      expect(
        firstType.typeArguments.map<String>((ElementType t) => t.name),
        orderedEquals(['int']),
      );
      expect(lastType.name, equals('GenericClass'));
      expect(
        lastType.typeArguments.map<String>((ElementType t) => t.name),
        orderedEquals(['int']),
      );
    });
    ...
    test('Verify non-overridden members have right canonical classes', () {
      ...
      var modifierMember = TypeInferenceMixedIn.instanceFields.named(
        'modifierMember',
      );
      var mixinMember = TypeInferenceMixedIn.instanceFields.named(
        'mixinMember',
      );
  ...
  group('Class', () {
    ...
    test('get methods', () {
      expect(
        Dog.instanceMethods.wherePublic.where((m) => !m.isInherited),
        hasLength(16),
      );
    });
    ...
    test('exported class should have hrefs from the current library', () {
      expect(
        Dep.href,
        equals('${htmlBasePlaceholder}ex/Deprecated-class.html'),
      );
      expect(
        Dep.instanceMethods.named('toString').href,
    ...
    test(
      'ExtendingClass has a super class that is also in the same library',
      () {
        ...
        expect(
          ExtendingClass.superChain.first.modelElement.isCanonical,
          isTrue,
        );
    ...
    test(
      ...
      () {
        expect(
          ExtendingClass.superChain.last.name,
          equals('WithGetterAndSetter'),
        );
        expect(
          ExtendingClass.superChain.last.modelElement.library!.name,
          equals('fake'),
        );
      },
    );
  ...
  group('Comment References link tests', () {
    group('Linking for generalized typedef cases works', () {
      ...
      test('Verify ability to link to class members of aliased classes', () {
        expect(
          referenceLookup(generalizedTypedefs, 'T8.a'),
    ...
    group('Linking for complex inheritance and reexport cases', () {
      ...
      setUp(() {
        ...
        local_scope = packageGraph.libraries.named(
          'two_exports.src.local_scope',
        );
        ...
        aConstructor = BaseWithMembers.constructors.named(
          'BaseWithMembers.aConstructor',
        );

        someConflictingNameSymbol = extending.properties.named(
          'someConflictingNameSymbol',
        );
        ...
        aNotReexportedVariable = local_scope.properties.named(
          'aNotReexportedVariable',
        );
        anotherNotReexportedVariable = local_scope.properties.named(
          'anotherNotReexportedVariable',
        );
        aSymbolOnlyAvailableInExportContext = two_exports.properties.named(
          'aSymbolOnlyAvailableInExportContext',
        );
        someConflictingNameSymbolTwoExports = two_exports.properties.named(
          'someConflictingNameSymbol',
        );
      });
      ...
      test('Grandparent override in container members', () {
        expect(
          referenceLookup(aField, 'aNotReexportedVariable'),
        ...
        // feature.
        expect(
          referenceLookup(anotherField, 'aNotReexportedVariable'),
          equals(MatchingLinkResult(aNotReexportedVariable)),
        );
      });
      ...
      test('Linking for static/constructor inheritance across libraries', () {
        expect(
          referenceLookup(ExtendingAgain, 'aStaticField'),
          equals(MatchingLinkResult(aStaticField)),
        );
        expect(
          referenceLookup(ExtendingAgain, 'aStaticMethod'),
          equals(MatchingLinkResult(aStaticMethod)),
        );
        expect(
          referenceLookup(ExtendingAgain, 'aConstructor'),
      ...
      test('Linking for inherited field from reexport context', () {
        expect(
          referenceLookup(aField, 'anotherNotReexportedVariable'),
      ...
      test(
        'Link to definingLibrary for class rather than its export context',
        () {
          expect(
            referenceLookup(ExtendingAgain, 'someConflictingNameSymbol'),
            equals(MatchingLinkResult(someConflictingNameSymbol)),
          );
          expect(
            referenceLookup(two_exports, 'someConflictingNameSymbol'),
    ...
    group('Type parameter lookups work', () {
      ...
      setUpAll(() {
        aTopLevelTypeParameterFunction = fakeLibrary.functions.named(
        ...
        typedParam = aTopLevelTypeParameterFunction.parameters.named(
          'typedParam',
        );
        ...
        BTypeParam = TypeParameterThings.typeParameters.named(
          'BTypeParam extends FactoryConstructorThings',
        );

        TypeParameterThingsExtended = fakeLibrary.classes.named(
          'TypeParameterThingsExtended',
        );
        aMethodExtended = TypeParameterThingsExtended.instanceMethods.named(
          'aMethod',
        );

        TypeParameterThingsExtendedQ = fakeLibrary.classes.named(
          'TypeParameterThingsExtendedQ',
        );
        aMethodExtendedQ = TypeParameterThingsExtendedQ.instanceMethods.named(
          'aMethod',
        );
      ...
      test('on inherited documentation', () {
        expect(
          referenceLookup(aMethodExtended, 'ATypeParam'),
          equals(MatchingLinkResult(ATypeParam)),
        );
        expect(
          referenceLookup(aMethodExtended, 'BTypeParam'),
          equals(MatchingLinkResult(BTypeParam)),
        );
        expect(
          referenceLookup(aMethodExtended, 'CTypeParam'),
        ...
        expect(
          referenceLookup(aMethodExtended, 'QTypeParam'),
          equals(MatchingLinkResult(null)),
        );

        // We get an inverse situation on the extendedQ class.
        expect(
          referenceLookup(aMethodExtendedQ, 'ATypeParam'),
          equals(MatchingLinkResult(null)),
        );
        expect(
          referenceLookup(aMethodExtendedQ, 'BTypeParam'),
          equals(MatchingLinkResult(null)),
        );
        expect(
          referenceLookup(aMethodExtendedQ, 'CTypeParam'),
          equals(MatchingLinkResult(null)),
        );
        expect(
          referenceLookup(aMethodExtendedQ, 'QTypeParam'),
      ...
      test('on classes', () {
        expect(
          referenceLookup(TypeParameterThings, 'ATypeParam'),
          equals(MatchingLinkResult(ATypeParam)),
        );
        expect(
          referenceLookup(TypeParameterThings, 'BTypeParam'),
          equals(MatchingLinkResult(BTypeParam)),
        );
        expect(
          referenceLookup(aName, 'ATypeParam'),
          equals(MatchingLinkResult(ATypeParam)),
        );
        expect(
          referenceLookup(aThing, 'BTypeParam'),
          equals(MatchingLinkResult(BTypeParam)),
        );
        expect(
          referenceLookup(aMethod, 'CTypeParam'),
          equals(MatchingLinkResult(CTypeParam)),
        );
        expect(
          referenceLookup(aParam, 'ATypeParam'),
          equals(MatchingLinkResult(ATypeParam)),
        );
        expect(
          referenceLookup(anotherParam, 'CTypeParam'),
          equals(MatchingLinkResult(CTypeParam)),
        );
      });
      ...
      test('on top level methods', () {
        expect(
          referenceLookup(aTopLevelTypeParameterFunction, 'DTypeParam'),
          equals(MatchingLinkResult(DTypeParam)),
        );
        expect(
          referenceLookup(typedParam, 'DTypeParam'),
          equals(MatchingLinkResult(DTypeParam)),
        );
      });
    });
    ...
    group('Ordinary namespace cases', () {
      ...
      setUpAll(() async {
        ...
        aFunctionUsingRenamedLib = fakeLibrary.functions.named(
          'aFunctionUsingRenamedLib',
        );
        Dart = packageGraph.libraries.named('Dart');
        DartPackage = packageGraph.packages.firstWhere((p) => p.name == 'Dart');
        nameWithTwoUnderscores = fakeLibrary.constants.named(
          'NAME_WITH_TWO_UNDERSCORES',
        );
        nameWithSingleUnderscore = fakeLibrary.constants.named(
          'NAME_SINGLEUNDERSCORE',
        );
        string = packageGraph.libraries
            .named('dart:core')
            .classes
            .named('String');
        metaUseResult = packageGraph.libraries
            .named('meta')
            .classes
            .named('UseResult');
        baseForDocComments = fakeLibrary.classes.named('BaseForDocComments');
        aNonDefaultConstructor = baseForDocComments.constructors.named(
          'BaseForDocComments.aNonDefaultConstructor',
        );
        defaultConstructor = baseForDocComments.constructors.named(
          'BaseForDocComments.new',
        );
        somethingShadowyParameter = defaultConstructor.parameters.named(
          'somethingShadowy',
        );
        initializeMe = baseForDocComments.declaredFields.named('initializeMe');
        somethingShadowy = baseForDocComments.declaredFields.named(
          'somethingShadowy',
        );
        doAwesomeStuff = baseForDocComments.instanceMethods.named(
          'doAwesomeStuff',
        );
        anotherMethod = baseForDocComments.instanceMethods.named(
          'anotherMethod',
        );
        ...
        Apple = exLibrary.classes.named('Apple');
        incorrectDocReference = fakeLibrary.constants.named(
        ...
        BaseClass = packageGraph.libraries
            ...
            .classes
            .named('BaseClass');
        bracketOperator = baseForDocComments.instanceOperators.named(
          'operator []',
        );
        ...
        action = packageGraph.libraries
            ...
            .declaredFields
            .named('action');
        aConstructorShadowed = baseForDocComments.constructors.named(
          'BaseForDocComments.aConstructorShadowed',
        );
        aConstructorShadowedField = baseForDocComments.declaredFields.named(
          'aConstructorShadowed',
        );

        FactoryConstructorThings = fakeLibrary.classes.named(
          'FactoryConstructorThings',
        );
        anotherName = FactoryConstructorThings.constructors.named(
          'FactoryConstructorThings.anotherName',
        );
        anotherConstructor = FactoryConstructorThings.constructors.named(
          'FactoryConstructorThings.anotherConstructor',
        );
        ...
        anotherDifferentName = anotherName.parameters.named(
          'anotherDifferentName',
        );
        ...
        yetAnotherNameField = FactoryConstructorThings.declaredFields.named(
          'yetAnotherName',
        );
        initViaFieldFormal = FactoryConstructorThings.declaredFields.named(
          'initViaFieldFormal',
        );
      ...
      group('Parameter references work properly', () {
        test('via a setter with a function parameter', () {
          var aSetterWithFunctionParameter = fakeLibrary.properties.named(
            'aSetterWithFunctionParameter',
          );
          var fParam = aSetterWithFunctionParameter.parameters.named('fParam');
          var fParamA = (fParam.modelType as Callable).parameters.named(
            'fParamA',
          );
          var fParamB = (fParam.modelType as Callable).parameters.named(
            'fParamB',
          );
          var fParamC = (fParam.modelType as Callable).parameters.named(
            'fParamC',
          );
        ...
        test('in class scope overridden by fields', () {
          expect(
            referenceLookup(FactoryConstructorThings, 'aName'),
            equals(MatchingLinkResult(aNameField)),
          );
          var anotherNameField = FactoryConstructorThings.declaredFields.named(
            'anotherName',
          );
          expect(
            referenceLookup(FactoryConstructorThings, 'anotherName'),
            equals(MatchingLinkResult(anotherNameField)),
          );
          expect(
            referenceLookup(FactoryConstructorThings, 'yetAnotherName'),
          ...
          expect(
            referenceLookup(FactoryConstructorThings, 'redHerring'),
        ...
        test('in factory constructor scope referring to parameters', () {
          expect(
            referenceLookup(anotherName, 'aName'),
            equals(MatchingLinkResult(aName)),
          );
          expect(
            referenceLookup(anotherName, 'anotherName'),
            equals(MatchingLinkResult(anotherNameParameter)),
          );
          expect(
            referenceLookup(anotherName, 'anotherDifferentName'),
            equals(MatchingLinkResult(anotherDifferentName)),
          );
          expect(
            referenceLookup(anotherName, 'differentName'),
            equals(MatchingLinkResult(differentName)),
          );
          expect(
            referenceLookup(anotherName, 'redHerring'),
            equals(MatchingLinkResult(redHerring)),
          );
        });
        ...
        test('in factory constructor scope referring to constructors', () {
          ...
          expect(
            referenceLookup(anotherName, 'anotherConstructor'),
        ...
        test('in method scope referring to parameters and variables', () {
          expect(
            referenceLookup(aMethod, 'yetAnotherName'),
            equals(MatchingLinkResult(yetAnotherName)),
          );
          expect(
            referenceLookup(aMethod, 'FactoryConstructorThings.yetAnotherName'),
            equals(MatchingLinkResult(yetAnotherNameField)),
          );
          expect(
            referenceLookup(
              aMethod,
              'FactoryConstructorThings.anotherName.anotherName',
            ),
            equals(MatchingLinkResult(anotherNameParameter)),
          );
          expect(
            referenceLookup(aMethod, 'aName'),
            equals(MatchingLinkResult(aNameField)),
          );
        });
      });
      ...
      test(
        'Referring to libraries and packages with the same name is fine',
        () {
          expect(
            referenceLookup(Apple, 'Dart'),
            equals(MatchingLinkResult(Dart)),
          );
          expect(
            referenceLookup(Apple, 'package:Dart'),
      ...
      test('Verify basic linking inside a constructor', () {
        expect(
          referenceLookup(aNonDefaultConstructor, 'initializeMe'),
      ...
      test('Verify that constructors do not override member fields unless '
      ...
      test('Deprecated lookup styles still function', () {
        expect(
          referenceLookup(baseForDocComments, 'aPrefix.UseResult'),
      ...
      test('Verify basic linking inside class', () {
        ...
        expect(
          referenceLookup(baseForDocComments, 'somethingShadowy'),
          equals(MatchingLinkResult(somethingShadowy)),
        );
        ...
        expect(
          referenceLookup(doAwesomeStuff, 'aNonDefaultConstructor'),
          equals(MatchingLinkResult(aNonDefaultConstructor)),
        );
        ...
        expect(
          referenceLookup(doAwesomeStuff, 'ExtendedBaseReexported.action'),
  ...
  group('Extensions', () {
    ...
    setUpAll(() {
      reexportOneLib = packageGraph.libraries.named('reexport_one');
      reexportTwoLib = packageGraph.libraries.named('reexport_two');
      documentOnceReexportOne = reexportOneLib.extensions.named(
        'DocumentThisExtensionOnce',
      );
      documentOnceReexportTwo = reexportTwoLib.extensions.named(
        'DocumentThisExtensionOnce',
      );
      string = packageGraph.libraries
          .named('dart:core')
    ...
    test('basic canonicalization for extensions', () {
      expect(
        documentOnceReexportOne.href,
        equals(documentOnceReexportTwo.href),
      );
    ...
    test('classes know about applicableExtensions', () {
      ...
      expect(
        string.potentiallyApplicableExtensionsSorted,
        contains(documentOnceReexportTwo),
      );
      expect(baseTest.potentiallyApplicableExtensionsSorted, isEmpty);
      expect(
        anotherExtended.potentiallyApplicableExtensionsSorted,
        orderedEquals([uphill]),
      );
      expect(
        bigAnotherExtended.potentiallyApplicableExtensionsSorted,
        orderedEquals([uphill]),
      );
    });
    ...
    test('applicableExtensions include those from implements & mixins', () {
      ...
      expect(
        implementor2.potentiallyApplicableExtensionsSorted,
        orderedEquals([extensionCheckImplementor2, extensionCheckLeft]),
      );
      expect(
        school.potentiallyApplicableExtensionsSorted,
    ...
    test('type parameters and bounds work with applicableExtensions', () {
      expect(
        superMegaTron.potentiallyApplicableExtensionsSorted,
        orderedEquals([leg]),
      );
      expect(
        megaTron.potentiallyApplicableExtensionsSorted,
    ...
    test('extension name with generics', () {
      expect(
        ...
        equals('FancyList&lt;<wbr><span class="type-parameter">Z</span>&gt;'),
      );
    });

    test('extended type has generics', () {
      expect(
        fancyList.extendedElement.nameWithGenerics,
  ...
  group('Function', () {
    ...
    test('handles dynamic parameters correctly', () {
      expect(
        ParameterRendererHtml().renderLinkedParams(f1.parameters),
        contains('lastParam'),
      );
    });
    ...
    test('async function', () {
      ...
      expect(
        ...
        equals('An async function. It should look like I return a [Future].'),
      );
      expect(
        thisIsAsync.documentationAsHtml,
        equals(
    ...
    test('function with a parameter having type FutureOr<Null>', () {
      expect(
        ParameterRendererHtml().renderLinkedParams(
          paramOfFutureOrNull.parameters,
        ),
        equals(
    ...
    test('escapes HTML in default values', () {
      ...
      expect(
        ...
        contains(
          '<span class="parameter-name">p3</span> = '
...
String? topLevelFunction(int param1, bool param2, Cool coolBeans,
  ...
  return null;
    ...
    test('typedef params have proper signature', () {
      ...
      var params = ParameterRendererHtml().renderLinkedParams(
        function.parameters,
      );
      expect(
        params,
    ...
    test('supports generic methods', () {
      expect(
        genericFunction.nameWithGenerics,
    ...
    test('can resolve functions as parameters', () {
      var params = ParameterRendererHtml().renderLinkedParams(
        doAComplicatedThing.parameters,
      );
      expect(
        params,
  ...
  group('Method', () {
    ...
    setUpAll(() {
      ...
      inheritedClear = CatString.inheritedMethods.singleWhere(
        (m) => m.name == 'clear',
      );
      ...
      abstractMethod = Cat.instanceMethods.singleWhere(
        (m) => m.name == 'abstractMethod',
      );
      ...
      convertToMap = HasGenerics.instanceMethods.singleWhere(
        (m) => m.name == 'convertToMap',
      );
      TypedFunctionsWithoutTypedefs = exLibrary.classes.named(
        'TypedFunctionsWithoutTypedefs',
      );
    ...
    test('parameter has generics in signature', () {
      expect(
        testGeneric.parameters[0].modelType.linkedName,
    ...
    test('parameter is a function', () {
      ...
      expect(
        (functionArgParam.modelType as Callable).returnType.linkedName,
        'String',
      );
    });
    ...
    test('generic method type args are rendered', () {
      expect(
        testGenericMethod.nameWithGenerics,
  ...
  group('Operators', () {
    ...
    test(
      'if inherited and superclass not in package, link to enclosed class',
      () {
        expect(
          plus.linkedName,
          equals(
  ...
  group('Field', () {
    ...
    setUpAll(() {
      ...
      UnusualProperties = fakeLibrary.classes.named(
        'ClassWithUnusualProperties',
      );
      implicitGetterExplicitSetter =
          UnusualProperties.allModelElements.firstWhere(
                (e) => e.name == 'implicitGetterExplicitSetter',
              )
              as Field;
      explicitGetterImplicitSetter =
          UnusualProperties.allModelElements.firstWhere(
                (e) => e.name == 'explicitGetterImplicitSetter',
              )
              as Field;
      explicitGetterSetter =
          UnusualProperties.allModelElements.firstWhere(
                (e) => e.name == 'explicitGetterSetter',
              )
              as Field;
      finalProperty =
          UnusualProperties.allModelElements.firstWhere(
                (e) => e.name == 'finalProperty',
              )
              as Field;
    ...
    test('split inheritance with explicit setter works', () {
      ...
      expect(
        implicitGetterExplicitSetter.attributes,
    ...
    test('split inheritance with explicit getter works', () {
      ...
      expect(
        explicitGetterImplicitSetter.attributes,
        isNot(contains(Attribute.inherited)),
      );
      expect(
        explicitGetterImplicitSetter.attributes,
      ...
      expect(
        explicitGetterImplicitSetter.oneLineDoc,
    ...
    test(
      ...
      () {
        ...
        expect(
          withGenericSub.inheritedFields.where((p) => p.name == 'prop'),
          hasLength(1),
        );
      },
    );
  ...
  group('Accessor', () {
    ...
    test('always has a valid location', () {
      ...
      for (var a in macrosFromAccessors.inheritedFields.expand(
        expandAccessors,
      )) {
        if (a.name == 'values') {
          continue;
  ...
  group('Top-level Variable', () {
    ...
    setUpAll(() {
      ...
      importantComputations = fakeLibrary.properties.named(
    ...
    test('@nodoc on simple property works', () {
      var nodocSimple = fakeLibrary.properties.wherePublic.firstWhereOrNull(
    ...
    test('@nodoc on both hides both', () {
      var nodocBoth = fakeLibrary.properties.wherePublic.firstWhereOrNull(
  ...
  group('Constructor', () {
    ...
    setUpAll(() {
      ...
      constructorTesterDefault = constructorTester.constructors.named(
        'ConstructorTester.new',
      );
      constructorTesterFromSomething = constructorTester.constructors.named(
        'ConstructorTester.fromSomething',
      );
      referToADefaultConstructor = fakeLibrary.classes.named(
        'ReferToADefaultConstructor',
      );
      withSyntheticConstructor = exLibrary.classes.named(
        'WithSyntheticConstructor',
      );
      syntheticConstructor = withSyntheticConstructor.constructors.firstWhere(
        (c) => c.isUnnamedConstructor,
      );
    });
    ...
    test('displays generic parameters correctly', () {
      expect(
        constructorTesterDefault.nameWithGenerics,
        'ConstructorTester&lt;<wbr><span class="type-parameter">A</span>, <span class="type-parameter">B</span>&gt;',
      );
      expect(
        constructorTesterFromSomething.nameWithGenerics,
    ...
    test('has a line number and column', () {
      ...
      expect(
        appleDefaultConstructor.characterLocation.toString(),
        isNot(equals(apple.characterLocation.toString())),
      );
      // A synthetic class should reference its parent.
      expect(
        syntheticConstructor.characterLocation.toString(),
  ...
  group('void as type', () {
    ...
    setUpAll(() {
      ...
      ATypeTakingClassMixedIn = fakeLibrary.classes.named(
        'ATypeTakingClassMixedIn',
      );
    });
    ...
    test('a class that extends Future<void>', () {
      ...
      var FutureVoid = ExtendsFutureVoid.superChain.wherePublic.firstWhere(
        (c) => c.name == 'Future',
      );
      expect(
    ...
    test('a class that implements Future<void>', () {
      ...
      var FutureVoid = ImplementsFutureVoid.directInterfaces.firstWhere(
        (e) => e.name == 'Future',
      );
      expect(
        FutureVoid.linkedName,
  ...
  group('Typedef', () {
    ...
    test('TypedefRenderer renders genericParameters', () {
      expect(
        processMessage.genericParameters,
  ...
  group('Parameter', () {
    ...
    setUpAll(() {
      c = exLibrary.classes.named('Apple');
      CovariantMemberParams = fakeLibrary.classes.named(
        'CovariantMemberParams',
      );
      applyCovariantParams = CovariantMemberParams.instanceMethods.named(
        'applyCovariantParams',
      );
      paramFromExportLib = c.instanceMethods.singleWhere(
        (m) => m.name == 'paramFromExportLib',
      );
      isGreaterThan = c.instanceMethods.singleWhere(
        (m) => m.name == 'isGreaterThan',
      );
      ...
      methodWithGenericParam = fClass.instanceMethods.singleWhere(
        (m) => m.name == 'methodWithGenericParam',
      );
      methodWithTypedefParam = c.instanceMethods.singleWhere(
        (m) => m.name == 'methodWithTypedefParam',
      );
      doAComplicatedThing = fakeLibrary.functions.wherePublic.named(
        'doAComplicatedThing',
      );
    });
    ...
    test('covariant parameters render correctly', () {
      ...
      expect(
        applyCovariantParams.linkedParamsLines,
    ...
    test(
      ...
      () {
        expect(
          doAComplicatedThing.documentationAsHtml,
    ...
    test('uses = instead of : to set default value', () {
      final rendered = ParameterRendererHtml().renderLinkedParams([
        intCheckOptional,
      ]);
    ...
    test('param with generics', () {
      var params = ParameterRendererHtml().renderLinkedParams(
        methodWithGenericParam.parameters,
      );
    ...
    test('typedef param is linked and does not include types', () {
      var params = ParameterRendererHtml().renderLinkedParams(
        methodWithTypedefParam.parameters,
      );
      expect(
        params,
        equals(
          '<span class="parameter" id="methodWithTypedefParam-param-p">'
  ...
  group('Implementors', () {
    test(
      ...
      () {
        var GenericSuperProperty = fakeLibrary.classes.named(
          'GenericSuperProperty',
        );
        var publicImplementors = GenericSuperProperty.publicImplementersSorted
            .map((i) => i.name);
        ...
        publicImplementors = GenericSuperValue.publicImplementersSorted.map(
          (i) => i.name,
        );
        expect(publicImplementors, hasLength(1));
        // A direct implementor.
        ...
        publicImplementors = GenericSuperNum.publicImplementersSorted.map(
          (i) => i.name,
        );
        expect(publicImplementors, hasLength(1));
  ...
  group('Errors and exceptions', () {
    ...
    test('library has the exact errors/exceptions we expect', () {
      expect(
        exLibrary.exceptions.map((e) => e.name),
  ...
  group('Source Code HTML', () {
    ...
    setUpAll(() {
      EscapableProperties = fakeLibrary.classes.named(
        'HtmlEscapableProperties',
      );
      implicitGetterExplicitSetter =
          EscapableProperties.allModelElements.firstWhere(
                (e) => e.name == 'implicitGetterExplicitSetter',
              )
              as Field;
      explicitGetterImplicitSetter =
      ...
      simpleProperty =
          EscapableProperties.allModelElements.firstWhere(
                (e) => e.name == 'simpleProperty',
              )
              as Field;
      forInheriting =
          EscapableProperties.allModelElements.firstWhere(
                (e) => e.name == 'forInheriting',
              )
              as Field;
      explicitGetterSetterForInheriting =
          EscapableProperties.allModelElements.firstWhere(
                (e) => e.name == 'explicitGetterSetterForInheriting',
              )
              as Field;
      ensureWholeDeclarationIsVisible =
          EscapableProperties.allModelElements.firstWhere(
                (e) => e.name == 'ensureWholeDeclarationIsVisible',
              )
              as Field;
    });
    ...
    test('Implicit accessors are escaped', () {
      expect(
        implicitGetterExplicitSetter.getter!.sourceCode,
        contains('&lt;int&gt;'),
      );
      expect(
        implicitGetterExplicitSetter.setter!.sourceCode,
        contains('&lt;int&gt;'),
      );
      expect(
        explicitGetterImplicitSetter.getter!.sourceCode,
        contains('&lt;int&gt;'),
      );
      expect(
        explicitGetterImplicitSetter.setter!.sourceCode,
        contains('&lt;int&gt;'),
      );
      expect(
        explicitGetterSetterForInheriting.getter!.sourceCode,
        contains('&lt;int&gt;'),
      );
      expect(
        explicitGetterSetterForInheriting.setter!.sourceCode,
        contains('&lt;int&gt;'),
      );
    });
    ...
    test('Property fields are terminated with semicolon', () {
      expect(
        finalProperty.sourceCode.trim(),
        endsWith('List&lt;int&gt;.filled(1, 1);'),
      );
      expect(
        simpleProperty.sourceCode.trim(),
        endsWith('List&lt;int&gt;.filled(1, 1);'),
      );
    ...
    test('Arrow accessors are terminated with semicolon', () {
      expect(
        explicitGetterImplicitSetter.getter!.sourceCode.trim(),
        endsWith('List&lt;int&gt;.filled(1, 1);'),
      );
      expect(
        explicitGetterSetter.getter!.sourceCode.trim(),
        endsWith('List&lt;int&gt;.filled(1, 1);'),
      );
    });
    ...
    test('Traditional accessors are not terminated with semicolon', () {
      expect(
        implicitGetterExplicitSetter.setter!.sourceCode.trim(),
        endsWith('{}'),
      );
    ...
    test('Whole declaration is visible when declaration spans many lines', () {
      expect(
        ensureWholeDeclarationIsVisible.sourceCode,
        contains('List&lt;int&gt;? '),
      );
    });
  });
