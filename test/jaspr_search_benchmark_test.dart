...
void main() {
  test('benchmark search matches non-Latin queries with Unicode normalization',
      () {
    final tempDir = Directory.systemTemp.createTempSync('jaspr_benchmark_test.');
  ...
  test('benchmark prefers framework generic titles for common short queries', () {
    final tempDir = Directory.systemTemp.createTempSync('jaspr_benchmark_rank.');
    addTearDown(() => tempDir.deleteSync(recursive: true));
  ...
  test('benchmark prefers concise framework context types over long suffix matches',
      () {
    final tempDir =
        Directory.systemTemp.createTempSync('jaspr_benchmark_context.');
    addTearDown(() => tempDir.deleteSync(recursive: true));
  ...
  test('benchmark dedupes exact page hits ahead of same-page member noise', () {
    final tempDir =
        Directory.systemTemp.createTempSync('jaspr_benchmark_dedupe.');
    addTearDown(() => tempDir.deleteSync(recursive: true));
